...
template <typename C>
struct DBBind1<C, typename std::enable_if<std::is_integral<C>::value>::type> {
  void operator()(sqlite3_stmt &s, int idx, const C &value) {
    int res = sqlite3_bind_int64(&s, idx, value);
    if (res != SQLITE_OK) {
      throw DBException(res, "Binding parameter.");
    }
  }
};
...
inline void DBBindImpl(sqlite3_stmt & /*s*/, int /*idx*/) {}

template <typename T, typename... ARGS>
inline void DBBindImpl(sqlite3_stmt &s, int idx, const T &a, ARGS... args) {
  DBBind1<T>()(s, idx, a);
  DBBindImpl(s, idx + 1, args...);
}
...
struct ExtractImpl<T, ARGS...> {
  inline std::tuple<T, ARGS...> operator()(sqlite3 &db, sqlite3_stmt &row,
    ...
    if (err == SQLITE_NOMEM) {
      ...
      throw DBException(&db, std::string("Extracting value (") +
...
template <typename... ARGS>
void DBBind(sqlite3_stmt &s, ARGS... args) {
  detail::DBBindImpl(s, 1, args...);
}
