    ...
    ~TmpDir() {
        try {
            ...
            LOG(ERROR, std::string("Failed to remove temp test directory "
                    "because (") + e.what() + "), leaving garbage behind ("
                    + this->dir + ")");
        }
    }
...
public:
    SqliteTest() : db(tmp.dir + "/db.sqlite3"), data{
            std::make_tuple(1, 0.1, "one"),
...
TEST_F(SqliteTest, InputIterator) {
    this->CreateTable();
    this->InsertValues();
    auto res = this->db.Query<int, float, std::string>(
            "SELECT * FROM Tbl ORDER BY id;");
...
TEST_F(SqliteTest, Querying) {
    ...
    auto diff = std::mismatch(this->data.begin(), this->data.end(),
            res.begin());
...
TEST_F(SqliteTest, InsertFail) {
    ...
    EXPECT_THROW(
            this->db.SqliteExec(
                "INSERT INTO Tbl VALUES(4, 4.0, \"four\");"),
            sqlite_exception);
